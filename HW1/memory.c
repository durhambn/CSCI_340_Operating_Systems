...
int numberOfBlocks( unsigned int addr_bits, unsigned int num_block_offset_bits ) {
	...
	*/
	if( addr_bits >0 && num_block_offset_bits >0){
	double blocks;
	double value = 2;
	blocks = (pow(value, addr_bits)/pow(value, num_block_offset_bits));

	return blocks; // this is simply put here so the code complies, you must code correctly.
	}
	else{
	return FAIL;
	}
